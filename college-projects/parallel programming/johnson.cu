...
int main()
{
	...
	t1=clock();
	bellman_ford(graph,k,edges,node_dist,parent_node);
	...
	dim3 ngrid(1,1,1);
	dim3 nblock(VERTICES,1,1);

	cudaEventRecord(start);
	for(i=0;i<V;i++)
	{
		closestNode<<<1,1>>>(gpu_node_dist,gpu_visited,gpu_closest_vertex,VERTICES);
